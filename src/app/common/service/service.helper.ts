...
export class ServiceHelper {
    ...
    public api(): IRequestBuilder {
        ...
        return new RequestBuilder(this._http, environment.URL_API, new UserTokenProvider(this), this.fetchStart, this.fetchEnd);
    }
}
...
export class RequestBuilder implements IRequestBuilder {
    ...
    sendFile(payload: Array<FileUploaderModel>, timenow: any, progress: any): Observable<any> {
        ...
        return new Observable(observer => {
          ...
          // Define request state change event listener
          xhr.onreadystatechange = () => {
            if (xhr.readyState === XMLHttpRequest.DONE) {
              if (xhr.status === 200) {
                // Save response to local storage
                localStorage.setItem('imageUrl', xhr.response);
                observer.next(JSON.parse(xhr.response)); // Emit successful response
                observer.complete(); // Complete the Observable
              } else {
                // Emit error response
                observer.error(JSON.parse(xhr.response || '{ "message": "An error occurred" }'));
              }
            }
          };
